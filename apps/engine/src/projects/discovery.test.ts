...
import { makeRepositoryFixture } from "../../test/repository-fixture.js";
import { discoverRepository, requireRepositoryDirectory, slugify } from "./discovery.js";
...
describe("requireRepositoryDirectory", () => {
  it("refuses anything that is not an existing absolute directory", () => {
    for (const value of ["", "   ", "relative/path", "/definitely/not/here", 42, null]) {
...
describe("discoverRepository", () => {
  ...
  it("prefers the origin remote over other remotes", () => {
    ...
    expect(discoverRepository(root).remoteUrl).toBe("git@github.com:mine/repo.git");
  });
