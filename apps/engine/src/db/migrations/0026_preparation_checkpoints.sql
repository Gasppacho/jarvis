-- Ticket #190: worktree preparation is durable, observable execution progress.
CREATE TABLE execution_checkpoints_with_preparation (
  project_id TEXT NOT NULL REFERENCES projects (id) ON DELETE CASCADE,
  execution_id TEXT NOT NULL,
  sequence INTEGER NOT NULL CHECK (sequence > 0),
  source_sequence INTEGER NOT NULL CHECK (source_sequence > 0),
  type TEXT NOT NULL CHECK (type IN ('preparation.started', 'preparation.completed', 'preparation.failed', 'agent.started', 'agent.message', 'validation.started', 'validation.failed', 'commit.created', 'branch.pushed')),
...
INSERT INTO execution_checkpoints_with_preparation
...
ALTER TABLE execution_checkpoints_with_preparation RENAME TO execution_checkpoints;
