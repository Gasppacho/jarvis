...
describe("repository discovery and project import", () => {
  ...
  const discover = (engine: EngineHandle, path: string) =>
  ...
  const importProject = (engine: EngineHandle, body: Record<string, unknown>) =>
  ...
  it("reports a plain directory as not a git repository", async () => {
    ...
    const body = (await (await discover(engine, root)).json()) as Record<
      string,
      unknown
    >;
  ...
  it("refuses to import the same repository twice", async () => {
    ...
    expect((await importProject(engine, { repositoryPath: root })).status).toBe(
      201,
    );
  ...
  it("adopts an existing .jarvis/project.yaml", async () => {
    ...
    const root = fixture(() =>
      makeNodeRepositoryFixture({ projectYaml: committed }),
    );
  ...
  it("understands a linked worktree, whose .git is a file", async () => {
    ...
    const body = (await (await discover(engine, worktree)).json()) as Record<
      string,
      unknown
    >;
    expect(validateDiscovery(body), explain(validateDiscovery)).toBe(true);
  ...
  it("recognises the same repository reached through a symlink", async () => {
    ...
    expect((await importProject(engine, { repositoryPath: real })).status).toBe(
      201,
    );
    ...
    expect(
      ((await viaSymlink.json()) as { error: { code: string } }).error.code,
    ).toBe("project.already-imported");
  });
  ...
  it("refuses an oversized committed configuration instead of ignoring it", async () => {
    ...
    expect(
      ((await response.json()) as { error: { code: string } }).error.code,
    ).toBe("project.config-invalid");
  });

  it("serves a degraded health document and 503s projects when the database fails", async () => {
    ...
    const engine = await start({ dataRoot: join(asFile, "data") });

    const health = (await (await engine.call("/v1/health")).json()) as Record<
      string,
      unknown
    >;
    expect(health).toMatchObject({ status: "degraded", database: "failed" });

    const projects = await engine.call("/v1/projects");
    expect(projects.status).toBe(503);
    expect(
      ((await projects.json()) as { error: { code: string } }).error.code,
    ).toBe("engine.database-unavailable");
  });
  ...
  it("keeps imported projects across an engine restart", async () => {
    ...
    try {
      ...
      const created = (await (
        await importProject(first, { repositoryPath: root })
      ).json()) as {
        id: string;
      };
      await first.stop();
